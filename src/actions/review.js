...
export const getReviews = () => async dispatch => {
  try {
    const res = await axios.get(
      `${process.env.GATSBY_SERVER_HOST_ROOT}/api/v1/reviews`,
    );
...
export const getReviewForSlug = (productSlug, sort) => async dispatch => {
  try {
    const res = await axios.get(
      `${process.env.GATSBY_SERVER_HOST_ROOT}/api/v1/reviews/?&sort=${sort}&productSlug=${productSlug}`,
    );
    dispatch({
      type: GET_REVIEWS,
      payload: res.data,
...
export const addReviewForSlug = (formData, productSlug) => async dispatch => {
  ...
  try {
    const res = await axios.post(
      `${process.env.GATSBY_SERVER_HOST_ROOT}/api/v1/reviews/review/${productSlug}`,
      formData,
      config,
    );
...
export const editReview = (formData, reviewId) => async dispatch => {
  ...
  try {
    const res = await axios.put(
      `${process.env.GATSBY_SERVER_HOST_ROOT}/api/v1/reviews/review/${reviewId}`,
      formData,
      config,
    );
    dispatch({
...
export const addLike = reviewId => async dispatch => {
  try {
    const res = await axios.put(
      `${process.env.GATSBY_SERVER_HOST_ROOT}/api/v1/reviews/like/${reviewId}`,
    );
...
export const removeLike = reviewId => async dispatch => {
  try {
    const res = await axios.put(
      `${process.env.GATSBY_SERVER_HOST_ROOT}/api/v1/reviews/unlike/${reviewId}`,
    );
    dispatch({
      type: UPDATE_LIKES,
      payload: { reviewId, likes: res.data },
